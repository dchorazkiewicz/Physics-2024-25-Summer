...
- Electric potential: Volt $[V=W/A=J/s/A=kg m^2/ s^3 /A]$

- Electric resistance: Ohm $[\Omega=V/A=kg\, m^2 / C^2 s^3=A^2 s^3 / kg\,m^2]$
...
### Uncertentity from total derivative
...
To estimate the uncertainty in measurement using the total derivative, follow these steps:
...
By following these steps and using the total derivative to estimate the uncertainty in a measurement, you can make more accurate and reliable measurements and improve the quality of your experimental data.

### Quantifying uncertainty using the total derivative

The *total derivative* is a mathematical concept that describes how a function changes with respect to changes in its variables. For example 

$$d f(x,y) = \frac{\partial f}{\partial x} dx + \frac{\partial f}{\partial y} dy
$$ 
 In the context of measurement, the total derivative can be used to quantify the uncertainty in a physical quantity due to uncertainties in the variables that affect it. 
...
### Example: Estimating Uncertainty in Volume Measurement of a Ball

The diameter of a certain ball was measured to be $L = 42 \pm 1$ cm. Using this measurement, estimate the volume of the ball $V$, and then calculate the uncertainty in this volume estimation $\Delta V$.
...
### Example: Calculating Resistance and Its Uncertainty
...
| Measurement n | Delivery time t_n (min) |
|---------------|-------------------------|
| 1             | 30                      |
| 2             | 25                      |
| 3             | 31                      |
...
$$\small
\text{Average delivery time} = \frac{1}{N} \sum_{n=1}^{N} t_n = \frac{1}{10} (30 + 25 + 31 + 40 + 23 + 45 + 30 + 25 + 35 + 47) = 32.6 \text{ minutes}
$$

**Standard Deviation:**

$$\small
\text{Standard deviation} = \sqrt{\frac{1}{N-1} \sum_{n=1}^{N} (t_n - \bar{t})^2} = \sqrt{\frac{(30 - 32.6)^2 + (25 - 32.6)^2 + \ldots + (47 - 32.6)^2}{9}} \approx 8.9 \text{ minutes}
$$
...
1. **Observation in Syene (modern-day Aswan, Egypt):**
...
The Earth's mass can be calculated using a pendulum and Newton's law of gravitation:
...
Link: ![](https://commons.wikimedia.org/wiki/File:End_of_universe.jpg#/media/File:End_of_universe.jpg)
